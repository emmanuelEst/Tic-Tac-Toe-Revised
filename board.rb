...
class Board
  ...
  def generate_row(last_row = false)
    ...
    if last_row == true 
      puts Array.new(5) { |i| i % 2 == 0 ? ' ' : '|' }.join('')
    else
      arr = Array.new(5) { |i| i % 2 == 0 ? "\e[4m \e[0m" : "\e[4m|\e[0m" } # when i is even '_' will be added | when i is not even '|' is added
      puts arr.join('')
    end
  end
  ...
  def generate_board
    ...
    puts "\n"
    3.times {|i| i == 2 ? generate_row(true) : generate_row}
    puts "\n"
  end
end